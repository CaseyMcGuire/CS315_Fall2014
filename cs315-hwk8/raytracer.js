...
Camera.prototype.castRay = function(x, y){
    var u = (this.w * x/(canvas.width - 1)) - (this.w/2.0); //changes coord system
 ...
 */
 function colorEachPixelNormally(){
     ...
     console.log("color each pixel normally");
     for(var x = 0; x < canvas.width; x++){
	 for(var y = 0; y < canvas.height; y++){
...
*/
 function colorUsingDepthOfField(){
	 ...
	 for(var y = 0; y < canvas.height; y++){
	     ...
	     //intersect one of the two triangles that make up the focal plane
	     var focalPoint = focalPlane[0].intersects(curRay);
	     if(focalPoint === null){
		 focalPoint = focalPlane[1].intersects(curRay);
	     }

	     focalPoint = focalPoint.intersectionPoint;

	     var pixelColor = vec3.create();
...
function getRandomLocationsOnLense(){
	...
	var y = Math.sqrt(r) * Math.sin(theta);
	array[i] = [x,y,1];
    }
